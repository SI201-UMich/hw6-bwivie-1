...
def update_cache(breed_ids, cache_file):
    ...
    percentage = (added / len(breed_ids)) * 100
    ...
    for entry in cache.values():
        try:
            ...
            if not isinstance(max_life, int):
                continue
            if best_max is None or max_life > best_max or (max_life == best_max and name < best_name):
                best_name = name
                best_max = max_life
...
def get_groups_above_cutoff(cutoff, cache_file):
    ...
    for entry in cache.values():
        try:
            ...
            continue
    return {k: v for k, v in counts.items() if v >= cutoff}
...
def recommend_breeds_in_same_group(breed_name, cache_file):
    ...
    for entry in cache.values():
        try:
            name = entry["data"]["attributes"]["name"]
            gid = entry["data"]["relationships"]["group"]["data"]["id"]
            if gid == target_group and name.lower() != breed_name.lower():
                matches.append(name)
        except (KeyError, TypeError):
            continue
